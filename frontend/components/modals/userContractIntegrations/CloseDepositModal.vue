		...
		<div class="add-funds-card">
			...
			<div class="is-flex is-align-items-flex-start mb-60 mw-600">
				<ValidationProvider
					...
					name="amount"
				>
					<base-input type="number" v-model="depositBalance" disabled size="4" class="is-flex-grow-1" />
				</ValidationProvider>
...
export default {
	...
	methods: {
		async action() {
			...
			if (isValid && this.isTermsAcceped) {
				...
				await this.$store.dispatch('userContractIntegration/closeDeposit', parseInt(this.depositBalance))
				this.$parent.close()
			}
		},
		logKey(e) {
	...
	computed: {
		...mapGetters('userContractIntegration', ['depositBalance']),
		isTermsAcceped: {
			get() {
