<template lang="pug">
  div.repay-modal
    div.title.is-5.main-title Confirm pass repay
...
export default {
  ...
  methods: {
    async passRepay() {
      return await this.$store.dispatch("deposit/passRepayMany", this.paymentData).then(async isSuccess => {
        if (isSuccess) {
          await this.$store.dispatch('reports/updateAgreementPayment', this.ids)
  ...
  async created() {
    this.loading = true
    const resp = await this.$store.dispatch("reports/fetchAgreementPayment", this.fromTimestamp);
    this.loading = false
    if (!resp) {
    ...
    this.paymentData = {
      ...
      customerAddresses: resp.customerAddresses,
    }
    this.ids = resp.ids
    this.totalUSDT = resp.total
